...
class Driver(LabberDriver):
    ...
    # x_magnet_IP = "169.254.111.181"
    # x_magnet_IP = "169.254.199.31"
    x_magnet_IP = "169.254.199.101"
    x_magnet_PORT = 7180
    # y_magnet_IP = "169.254.141.159"
    # y_magnet_IP = "169.254.199.32"
    y_magnet_IP = "169.254.199.102"
    y_magnet_PORT = 7180
    # z_magnet_IP = "169.254.31.80"
    #z_magnet_IP = "169.254.199.33"
    #z_magnet_PORT = 7180

    BUFFSIZE = 1024
    CONNECTED_MAGNETS = {"x": False, "y": False}
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name[0] == "x":
            ...
            fieldrating = 1.0  # field rating is a maximum value that field can go to and its different for all magnets
        elif quant.name[0] == "y":
            fieldrating = 1.0
    ...
    def _send_opc(self, magnet_socket):
        """
        Send the *OPC? command to a magnet specified by its socket. The instrument will respond with 1 after
        executing the command. This is a computer ignostic way of making sure the previous command you wanted
        to execute actually got executed.
        ...
        """
        magnet_socket.sendall(bytes("*OPC?\n","ASCII"))
        time.sleep(0.1) # Horror
        return int(self._receive(magnet_socket)) # responding with 1 when executing *OPC?
    ...
    def _ask(self, magnet_socket, command):
        ...
        response = self._receive(magnet_socket)
        return response
    ...
    def set_field(self, magnet_socket, value):
        ...
        while self.get_ramp_state(magnet_socket) == 0:
            time.sleep(0.1)
        return True
    ...
    def start_ramping_algorithm(self, target_x, target_y, target_z):
        ...
        restart_all_threshold = 0.9  # When the field falls to 90% of max value, restart ramping
        # Stop treshold should be at 0.95 (for testing purposes it is changed to whatever number i find suitable)
        stop_threshold = 0.95  # When the field reaches close to 1 Tesla, stop magnets that are increasing it
        x = self.get_field(self.x_magnet_socket)
        y = self.get_field(self.y_magnet_socket)
        z = self.get_field(self.z_magnet_socket)
        radius, phi, theta = self.cartesian_to_spherical(x, y, z)
